...
namespace NanoRecvDataBase
{
    public partial class Form1 : Form
    {
        ...
        private void btnrecvErrorLog_Click(object sender, EventArgs e)
        {
            try
            {
                if (pairSocket == null)
                ...
                var res = new Task<string>(() =>
                {
                    bool ressend = pairSocket.SendImmediate(CreateTransBytes(CTransStatus.GetErrorLogName));
                    if (ressend)
                    {
                        while (true)
                        {
                ...
                res.Start();
                var getdbnum = res.Result;
                var lognames = getdbnum.Split('#');
                TextShow("接收到内存日志文件个数：" + lognames.Length);
                ...
                var resfile = new Task<String>(() =>
                {
                    for (int i = 0; i < lognames.Length; ++i)
                    {
                        ...
                        bool ressend = pairSocket.SendImmediate(CreateTransBytes(CTransStatus.TransLogFile, filename));
                        if (ressend)
                        {
                            while (true)
                            {
